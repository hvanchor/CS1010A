...
def monthly_max(fname, location, year):
    data = filter(lambda x: x[0] == location and x[1] == str(year), read_csv(fname)[1:])
    d = {}
    for row in data:
        month = row[2]
        max = float(row[5])
...
def windest_location(fname, year):
    ...
    for row in data:
        ...
        if month not in d:
            d[month] = {}
        if location not in d[month]:
            d[month][location] = []
        d[month][location].append(daily)
    for month,location in d.items():
        for loc,daily in location.items():
            location[loc] = round(sum(daily)/len(daily),2)
        d[month] = (max(location.items(), key=lambda x: x[1]))
    return d
...
class Pilot:
    def __init__(self, name, compatibility):
        self.name = name
    ...
    def train(self, pilot):
        if pilot == self:
            ...
            if pilot not in self.partners:
                self.partners[pilot] = 0
            self.partners[pilot] += 1
            if self not in pilot.partners:
                pilot.partners[self] = 0
            pilot.partners[self] += 1
            return f"{self.name} trains with {pilot.name}"

    def show_partners(self):
        return tuple(map(lambda x: (x[0].name, x[1]), self.partners.items()))
    ...
    def alight(self):
        if self.jaeger:
            sentence =  f"{self.name} alights from {self.jaeger.name}"
            self.jaeger.pilots.remove(self)
            self.jaeger = None
            return sentence
        else:
    ...
    def drift(self):
        ...
        for pilot in self.pilots:
            for partner in self.pilots:
                if pilot == partner:
                    continue
                if partner not in pilot.partners or pilot.partners[partner] < max(pilot.compatibility, partner.compatibility):
