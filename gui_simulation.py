...
GRID_PATH = "resources/grid.png"
VOID_PATH = "resources/void.png"

class Grid(cocos.sprite.Sprite):
    MAX_OBJECT = 16

    def __init__(self, row, col, resource=GRID_PATH):
        super(Grid, self).__init__(resource)
    ...
    def get_position_for_game_sprite_at_index(self, index):
        ...
        subRow, subCol = None, None
        for i in range(len(GRID_MAP)):
            for j in range(len(GRID_MAP[i])):
                if GRID_MAP[i][j] == index:
                    subRow = i
                    subCol = j
        if subRow == None or subCol == None:
            raise Exception('Too many objects here, consider less objects in the simulation or a bigger map :)')
...
class GameSprite(cocos.sprite.Sprite):
    ...
    def get_sprite_name(game_object):
        if isinstance(game_object, hgc.Food) and "meat" in game_object.get_name():
            ...
            sprite_name = "ai" + str(8 - (8 - GameSprite.tribute_id % 8) % 8) + ".png"
...
class GameReplayLayer(cocos.layer.Layer):
    def __init__(self, layout):
        ...
        self.layout = layout
        self.create_grids(layout)

    def create_grids(self, layout):
        for i in range(len(layout)):
            row = []
            for j in range(len(layout[0])):
                grid = Grid(i, j, [VOID_PATH, GRID_PATH][layout[i][j]])
    ...
    def get_grid_from_place(self, place):
        ...
        return self.grids[len(self.layout)-int(tokens[0])][int(tokens[1]) - 1]
...
def animate_game_logger_events(layout, callback=None):
    animate_game_events(layout, hgc.GAME_LOGGER.events, hgc.GAME_LOGGER.map_states, callback)

def animate_game_events(layout, game_events, map_states, callback=None):
    print("\n-----GUI Simulation-----")
    grid_width, grid_height = 128, 128
    cocos.director.director.init(width=len(layout[0]) * grid_width, height=len(layout) * grid_height, autoscale=False)
    game_replay_layer = GameReplayLayer(layout)
