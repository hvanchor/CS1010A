...
class Weapon(Thing):
    ...
    def json_output(self, *args, **kargs):
        output = super().json_output(*args, **kargs)
        output['max_dmg'] = self.max_dmg
        output['min_dmg'] = self.min_dmg
        return output
...
class Tribute(Person):
    ...
    def eat(self, food):
        if food in self.get_inventory() and isinstance(food, Food):
...
class GameLogger(object):
    ...
    def print_event(self, event, *args):
        ...
        if event in stringify:
            ...
            raise Exception(f'{event}: {args[1]}')
    ...
    def warning(self, warning):
        # Instead of printing the warning, we shall raise an exception instead
