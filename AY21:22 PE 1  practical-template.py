...
def num_subscribers(filename, event, resource):
   data = list(filter(lambda x: x[0] == event and x[1] == resource, read_csv(filename)))
   d = {}
   count = 0
   for row in data:
       if row[2] not in d:
...
def top_k_avg_emails(filename, start_date, end_date, k):
    data = filter(lambda x: start_date <= x[3] <= end_date, read_csv(filename)[1:])
    ...
    for key,val in d.items():
        d[key] = round(val/days, 2)
    emails = sorted(d.items(), key=lambda x: x[1], reverse=True)
    if k > len(emails):
        return emails
    kth_value = emails[k-1][1]
    return list(filter(lambda x: x[1] >= kth_value, emails))
...
class Game:
    ...
    def count_votes(self):
        voted = len(tuple(filter(lambda x: x.voted, self.players)))
        no = len(self.players) - voted
        if voted > no:
